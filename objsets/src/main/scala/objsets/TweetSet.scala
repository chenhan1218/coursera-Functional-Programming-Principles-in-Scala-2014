...
abstract class TweetSet {
  ...
  def mostRetweetedAcc(pivot: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweetedAcc(pivot: Tweet): Tweet = pivot
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    left.union(right).mostRetweetedAcc(elem)
  }
  def mostRetweetedAcc(pivot: Tweet): Tweet = {
    if (elem.retweets > pivot.retweets) left.union(right).mostRetweetedAcc(elem)
    else left.union(right).mostRetweetedAcc(pivot)
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(
    (t:Tweet)=> google.exists((it:String)=>t.text.contains(it))
  )
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(
    (t:Tweet)=> apple.exists((it:String)=>t.text.contains(it))
  )
